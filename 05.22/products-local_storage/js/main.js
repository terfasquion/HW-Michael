...
const Products = [
  ...
  new Product(
    ...
    "Popeye knows his business! Get your spinach today!",
    3.59,
    "spinach.jpg"
  ),
];
...
let ProductsFromDB = [];
const productsFromLS = localStorage.products;
if (
  productsFromLS &&
  productsFromLS.startsWith("[") && // Making sure productsFromLS is an array
  productsFromLS.endsWith("]")
)
  ProductsFromDB = JSON.parse(productsFromLS);
if (!ProductsFromDB.length) ProductsFromDB = Products.slice();
localStorage.products = JSON.stringify(ProductsFromDB);
...
ProductsFromDB.forEach(createArticle);
...
function deleteItem() {
  if (
    confirm(
      `Are you sure you want your ${getProdName(
        this
      )} to be permanently removed from your account?`
    )
  ) {
    ...
    for (let i = 0; i < ProductsFromDB.length; i++) {
      const currentProd = ProductsFromDB[i];
      if (currentProd.id === itemId) {
        ProductsFromDB.splice(i, 1);
        localStorage.products = JSON.stringify(ProductsFromDB);
        break;
      }
    }
